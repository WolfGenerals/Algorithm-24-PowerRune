...
class PowerRuneNode final : public rclcpp::Node {
    ...
    Subscription<CameraInfo>::SharedPtr cameraInfo_subscriber =
            create_subscription<CameraInfo>(
                "camera_info",
                10,
                [this](const CameraInfo::SharedPtr cameraInfo) -> void {
                    ...
                    distCoeffs = {
                                ...
                                static_cast<float>(rawDistCoeffs[1]),
                                static_cast<float>(rawDistCoeffs[2]),
                                static_cast<float>(rawDistCoeffs[3]),
                                static_cast<float>(rawDistCoeffs[4])
                            };
                }
            );
...
public:
    explicit PowerRuneNode(): Node("power_rune") { RCLCPP_INFO(get_logger(), "power_rune main start"); }
};
